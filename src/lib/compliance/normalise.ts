...
export function canonicaliseCategory(category: string) {
  ...
  return CAT_ALIASES[n] ? CAT_ALIASES[n].replace(/\b\w/g, m => m.toUpperCase()) : category;
}

export function canonicaliseTitle(title: string) {
  const n = normaliseText(title);
  const can = TITLE_ALIASES[n] ?? title;
  return can.replace(/\bEicr\b/,'EICR')
            .replace(/\bFra\b/,'FRA')
            .replace(' en 81-28', ' EN 81-28');
}
...
export function deriveFrequencyLabel(months?: number|null, label?: string|null) {
  ...
  const map: Record<number,string> = {
    1:'Monthly',3:'Quarterly',6:'6-Monthly',12:'Annual',24:'2-Yearly',36:'3-Yearly',60:'5-Yearly',120:'10-Yearly'
  };
