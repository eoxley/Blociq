...
export default function MajorWorksLabClient() {
  ...
  const fetchJobs = async () => {
    try {
      console.log('🔄 fetchJobs called - Fetching major works jobs from server...');
      const response = await fetch('/api/major-works-lab/jobs');
      console.log('📡 fetchJobs response status:', response.status);
      if (response.ok) {
        const data = await response.json();
        console.log('📋 Major works jobs fetched from server:', data.jobs?.length || 0, 'jobs');
