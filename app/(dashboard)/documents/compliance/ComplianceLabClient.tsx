...
import { Upload, FileText, Clock, CheckCircle, AlertCircle, Eye, Shield } from 'lucide-react';
...
export default function ComplianceLabClient() {
  ...
  const fetchJobs = async () => {
    try {
      console.log('🔄 fetchJobs called - Fetching compliance jobs from server...');
      const response = await fetch('/api/compliance-lab/jobs');
      console.log('📡 fetchJobs response status:', response.status);
      if (response.ok) {
        const data = await response.json();
        console.log('📋 Compliance jobs fetched from server:', data.jobs?.length || 0, 'jobs');
        setJobs(data.jobs || []);
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No compliance analyses yet</h3>
            <p className="text-gray-500">Upload EICRs, Fire Risk Assessments, or compliance certificates.</p>
            <p className="text-sm text-gray-400 mt-2">PDF documents up to 50 MB / 300 pages.</p>
          </div>
        ) : (
          <JobsList
            jobs={jobs}
            ...
            onRefresh={() => {
              console.log('🔄 onRefresh callback called from ComplianceLabClient');
              fetchJobs();
            }}
            category="compliance"
          />
