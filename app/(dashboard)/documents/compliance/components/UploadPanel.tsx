...
export default function UploadPanel({ onUploadSuccess }: UploadPanelProps) {
  ...
  const validateFile = (file: File): ValidationError | null => {
    // Check file type - Compliance documents are typically PDFs
    const allowedTypes = [
      'application/pdf',
    ...
    if (!allowedTypes.includes(file.type)) {
      return {
        type: 'type',
        message: "This file type isn't supported. Please upload PDF, DOCX, JPG, or PNG files."
      };
    }
    ...
    if (file.size > maxSize) {
      ...
      return {
        type: 'size',
        message: `File too large (${fileSizeMB}MB). Maximum size is ${maxSizeMB}MB. Please compress the file.`
      };
    }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/compliance-lab/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        setError({
          type: 'size',
          message: 'File too large for upload. Please compress the file.'
        });
      } else {
        try {
          const result = await response.json();
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-2">
                PDF, DOCX, or images. Max 50 MB and 300 pages.
              </p>
              <p className="text-xs text-gray-400 mt-1">
                EICRs, Fire Risk Assessments, Insurance documents, Certificates
              </p>
            </div>
          </div>
        )}
