...
export default function GeneralDocumentsPage() {
  ...
  const fetchGeneralDocuments = async () => {
    try {
      ...
      // Fetch general documents (exclude compliance and major works)
      const { data, error } = await supabase
        ...
        `)
        .not('type', 'ilike', '%compliance%')
        .not('type', 'ilike', '%eicr%')
        .not('type', 'ilike', '%fire%')
        .not('type', 'ilike', '%insurance%')
        .not('type', 'ilike', '%major%')
      ...
      console.error('Exception fetching general documents:', error)
      setDocuments([])
      toast.error('Failed to load general documents')
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredDocuments = documents.filter(doc => {
    ...
    const matchesFilter = filterType === 'all' ||
                         ...
                         doc.file_type.toLowerCase().includes(filterType.toLowerCase())
  ...
  const getDocumentIcon = (fileType: string, type: string) => {
    if (type.toLowerCase().includes('minute') || type.toLowerCase().includes('correspondence')) {
      return Mail
    }
  ...
  return (
    <div className="space-y-8">
      {/* Hero Banner */}
      <section className="relative overflow-hidden bg-gradient-to-r from-[#6b7280] to-[#4b5563] py-16 mx-6 rounded-3xl">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="text-center">
            ...
            <p className="text-xl text-white/90 max-w-3xl mx-auto leading-relaxed">
              Meeting minutes, correspondence, notices, and other general documentation
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search general documents..."
            value={searchTerm}
        ...
        {filteredDocuments.length === 0 ? (
          <BlocIQCard>
            <BlocIQCardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No general documents found</h3>
          ...
          filteredDocuments.map((document) => {
            const IconComponent = getDocumentIcon(document.file_type, document.type)
            return (
