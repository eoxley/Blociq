...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('✅ User authenticated for compliance lab upload');
    ...
    // Server-side validation for compliance documents
    ...
    const filePath = `compliance-lab/${fileName}`;
    ...
    if (uploadError) {
      console.error('Error uploading compliance file:', uploadError);
      return NextResponse.json({
        error: 'Upload failed',
    ...
    try {
      ...
      fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/compliance-lab/process`, {
        method: 'POST',
        headers: {
