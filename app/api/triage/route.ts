...
async function performSingleTriage(messageId: string) {
  try {
    ...
    const urgencyAnalysis = detectUrgency(emailContent);
    const propertyMentions = extractProperties(emailContent);
    const aiInsights = await generateAIInsights(emailContent, urgencyAnalysis, propertyMentions);
    const suggestedActions = generateSuggestedActions(emailContent, urgencyAnalysis, propertyMentions);
    const emailCategory = categorizeEmail(emailContent, urgencyAnalysis);

    const enhancedTriage = {
      // Keep existing AI categorization
      ...triageResult,
      
      // Add enhanced urgency detection
      ...
      enhancedCategory: emailCategory
    };
    ...
    try {
      await supabaseAdmin
        ...
        .upsert({
          ...
          triage_category: enhancedTriage.enhancedCategory,
          urgency_level: enhancedTriage.urgency.level,
          urgency_score: enhancedTriage.urgency.score,
          urgency_factors: enhancedTriage.urgency.factors,
          urgency_reasoning: enhancedTriage.urgency.reasoning,
...
// Enhanced triage functionality now integrated from @/lib/ai/enhancedTriage
