...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('🔄 Starting general document processing for job:', jobId);
    ...
    console.log('🔍 Starting OCR processing for general job:', jobId);
    ...
    console.log('✅ OCR completed for general job:', jobId);
    ...
    console.log('📄 Starting general document analysis for job:', jobId);

    // Call general document analysis service
    const analysisResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/ai/analyze-general`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!analysisResponse.ok) {
      console.error('❌ General document analysis failed for job:', jobId);
      await serviceSupabase
        .from('document_jobs')
        .update({
          status: 'FAILED',
          error_code: 'ANALYSIS_FAILED',
          error_message: 'General document analysis failed',
          updated_at: new Date().toISOString()
        })
        .eq('id', jobId);
    ...
    await serviceSupabase
      ...
      .update({
        status: 'READY',
        doc_type_guess: analysisResult.document_type || 'general_document',
        summary_json: analysisResult.summary,
        updated_at: new Date().toISOString()
      })
      .eq('id', jobId);

    console.log('🎉 General document processing completed successfully for job:', jobId);
    ...
    console.error('❌ Error in general document processing:', error);
