...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: existing } = await supabaseAdmin
    ...
    .select("compliance_asset_id").eq("building_id", params.id);
  const have = new Set((existing||[]).map((x:any)=>x.compliance_asset_id));
  const toAdd = (all||[]).map((a:any)=>a.id).filter((id:string)=>!have.has(id));
  if (!toAdd.length) return NextResponse.json({ inserted: 0 });
  const rows = toAdd.map((cid:string)=>({ building_id: params.id, compliance_asset_id: cid }));
