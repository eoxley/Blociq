...
export async function POST(req: NextRequest) {
  try {
    ...
    const filePath = `major-works-lab/${fileName}`;
    ...
    if (uploadError) {
      console.error('Error uploading major works file:', uploadError);
      return NextResponse.json({
        error: 'Upload failed',
    ...
    try {
      ...
      fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/major-works-lab/process`, {
        method: 'POST',
        headers: {
