...
export async function GET(req: NextRequest) {
  try {
    ...
    if (agencyId) {
      console.log('🔍 Fetching major works jobs by agency_id:', agencyId);
      const { data, error } = await supabase
        .from('document_jobs')
        .select('*')
        .eq('agency_id', agencyId)
        .eq('doc_category', 'major_works')
        .order('created_at', { ascending: false });

      jobs = data;
      jobsError = error;
    } else {
      console.log('🔍 No agency_id found, fetching major works jobs by user_id:', user.id);
      const { data, error } = await supabase
        .from('document_jobs')
        .select('*')
        .eq('user_id', user.id)
    ...
    if (jobsError) {
      console.error('Error fetching major works jobs:', jobsError);
      ...
      return NextResponse.json({
        ...
        message: 'Unable to retrieve major works jobs. Please try again.',
        details: process.env.NODE_ENV === 'development' ? jobsError.message : undefined
      }, { status: 500 });
    }

    console.log('📋 Major works jobs fetched:', jobs?.length || 0, 'jobs');
    ...
    console.error('Error in major works jobs API:', error);
    return NextResponse.json({
