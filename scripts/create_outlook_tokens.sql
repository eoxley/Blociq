...
CREATE POLICY IF NOT EXISTS "Users can view their own outlook tokens" ON outlook_tokens
  FOR SELECT USING (auth.uid() = user_id);

-- Users can insert their own tokens
CREATE POLICY IF NOT EXISTS "Users can insert their own outlook tokens" ON outlook_tokens
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Users can update their own tokens
CREATE POLICY IF NOT EXISTS "Users can update their own outlook tokens" ON outlook_tokens
  FOR UPDATE USING (auth.uid() = user_id);

-- Users can delete their own tokens
CREATE POLICY IF NOT EXISTS "Users can delete their own outlook tokens" ON outlook_tokens
...
DROP TRIGGER IF EXISTS update_outlook_tokens_updated_at ON outlook_tokens;
