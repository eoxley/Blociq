...
-- Enable Row Level Security (RLS) on document_jobs
ALTER TABLE document_jobs ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist (safe operation)
DROP POLICY IF EXISTS "Users can view their own jobs or agency jobs" ON document_jobs;
DROP POLICY IF EXISTS "Users can insert their own jobs" ON document_jobs;
DROP POLICY IF EXISTS "Users can update their own jobs or agency jobs" ON document_jobs;

-- Create RLS policies for document_jobs
CREATE POLICY "Users can view their own jobs or agency jobs" ON document_jobs
    FOR SELECT USING (
        user_id = auth.uid()
        OR
        agency_id IN (
...
CREATE POLICY "Users can insert their own jobs" ON document_jobs
    FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users can update their own jobs or agency jobs" ON document_jobs
    FOR UPDATE USING (
        user_id = auth.uid()
        OR
        agency_id IN (
...
-- Enable RLS on building_compliance_assets
ALTER TABLE building_compliance_assets ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view compliance assets for their buildings" ON building_compliance_assets;
DROP POLICY IF EXISTS "Users can manage compliance assets for their buildings" ON building_compliance_assets;

-- Create RLS policies for building_compliance_assets
CREATE POLICY "Users can view compliance assets for their buildings" ON building_compliance_assets
    FOR SELECT USING (
        building_id IN (
...
CREATE POLICY "Users can manage compliance assets for their buildings" ON building_compliance_assets
    FOR ALL USING (
        building_id IN (
            SELECT id FROM buildings WHERE
            user_id = auth.uid()
...
-- Enable RLS on compliance_assets (master list - read-only for all authenticated users)
ALTER TABLE compliance_assets ENABLE ROW LEVEL SECURITY;

-- Drop existing policy if it exists
DROP POLICY IF EXISTS "Authenticated users can view compliance assets" ON compliance_assets;

-- Create RLS policy for compliance_assets
CREATE POLICY "Authenticated users can view compliance assets" ON compliance_assets
...
BEGIN
    ...
    RAISE NOTICE 'Added RLS policies, indexes and triggers';
    RAISE NOTICE 'Ready for document lab functionality!';
END $$;