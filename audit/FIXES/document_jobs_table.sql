...
-- Create RLS policy for document_jobs - users can only see their own jobs or jobs from their agency
CREATE POLICY IF NOT EXISTS "Users can view their own jobs or agency jobs" ON document_jobs
    FOR SELECT USING (
        user_id = auth.uid()
        OR
        agency_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can insert their own jobs" ON document_jobs
    FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Users can update their own jobs or agency jobs" ON document_jobs
    FOR UPDATE USING (
        user_id = auth.uid()
        OR
        agency_id IN (
...
-- Create RLS policy for building_compliance_assets - users can only see assets for their buildings
CREATE POLICY IF NOT EXISTS "Users can view compliance assets for their buildings" ON building_compliance_assets
    FOR SELECT USING (
        building_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can manage compliance assets for their buildings" ON building_compliance_assets
    FOR ALL USING (
        building_id IN (
            SELECT id FROM buildings WHERE
            user_id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Authenticated users can view compliance assets" ON compliance_assets
...
RAISE NOTICE 'Added RLS policies and triggers for automatic timestamp updates';